...
public class MusicLoopsManager : Singleton<MusicLoopsManager> {
	...
	protected override void Awake()
	{
		...
		{
            m_MaxVolumes[i] = PlayerPrefs.GetFloat("MusicVolume");
        }
    ...
    private void Update() //menu paramètres
    {
        
        for (int i = 0; i < m_AudioSources.Length; i++)
        {
            if(m_AudioSources[i].volume != PlayerPrefs.GetFloat("MusicVolume"))
                m_AudioSources[i].volume = PlayerPrefs.GetFloat("MusicVolume");
            if (m_MaxVolumes[i] != PlayerPrefs.GetFloat("MusicVolume"))
                m_MaxVolumes[i] = PlayerPrefs.GetFloat("MusicVolume");
        }
    }
}
